...
if not os.path.exists(KEY_FILE):
    ...
    ope = OPE(os.urandom(16), in_range=ValueRange(0, 2**32-1), out_range=ValueRange(0, 2**64-1))
    with open(KEY_FILE, 'wb') as f:
        pickle.dump((pub, priv, ope.key), f)
else:
    with open(KEY_FILE, 'rb') as f:
        pub, priv, ope_key = pickle.load(f)
        ope = OPE(ope_key, in_range=ValueRange(0, 2**32-1), out_range=ValueRange(0, 2**64-1))
...
def main():
    while True:
        ...
        if choice == '1':
            ...
            print(f"Somme des salaires: {total}")
